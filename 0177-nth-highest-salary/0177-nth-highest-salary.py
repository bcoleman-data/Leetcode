...
def nth_highest_salary(employee: pd.DataFrame, N: int) -> pd.DataFrame:
   ...
   # salaries = employee["salary"].drop_duplicates().sort_values(ascending=False).reset_index(drop=True)
    ...
    if(len (employee["salary"].drop_duplicates()) < N or N <=0 ) : return pd.DataFrame({f"getNthHighestSalary({N})" : [None]})

    #pass f-string and iloc as dictionary as new dataframe pass salaries.iloc as array
    return pd.DataFrame({f"getNthHighestSalary({N})" : [employee["salary"].drop_duplicates().sort_values(ascending=False).iloc[N-1]]})